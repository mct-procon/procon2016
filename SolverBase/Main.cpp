...
bool AllSearch::dfs(int dep, const int maxdep) {
	...
	if (wakus[0].iscover(pieces[0])) isputted[0] = true;	//枠穴0の中にピース0が含まれてれば, if文内に入る。（ピース0が含まれているにも関わらず、if文内に入りません）
															//（原因：ピース0の頂点0と枠穴0の頂点0が殆ど同じ位置にあるので、ピース0の頂点0が枠穴0の外にあると判定してしまう）
	return true;
}
...
int WINAPI WinMain(HINSTANCE,HINSTANCE,LPSTR,int) {
	...
	while (ScreenFlip()==0 && ProcessMessage()==0 && ClearDrawScreen()==0 && CheckHitKey(KEY_INPUT_ESCAPE)==0) {
		solver.draw(false);	//isputted[i] = falseとなっているとき、ピースiは描画しないモード。
	}
	DxLib_End();
	return 0;
}
