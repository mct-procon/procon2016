...
private:
	...
	void draw(int is_piece) {
		...
		if (is_piece) { DrawFormatString(400, 25, GetColor(0, 0, 255), "ピースを作成中"); }
		else { DrawFormatString(400, 25, GetColor(0, 0, 255), "枠を作成中"); }
		DrawFormatString(400, 50, 0, "マウスで線を引く");
		DrawFormatString(400, 75, 0, "元に戻す：z, 完成：→");
	}

	//線分の集合をファイルに出力する関数 (長さを1～2伸ばして出力）
		...
		while (true) {
			...
			for (i = 0; i < n; i++) {
				for (j = 0; j < et[i].size(); j++) {
					...
					if (v != i) { continue; }
					if (is_piece && getArea(toPoints(cycle)) <= 0) { continue; }	//ピースなら, 時計回りはダメ
					if (!is_piece && getArea(toPoints(cycle)) >= 0) { continue; }	//枠なら, 反時計回りはダメ
					//頂点iを使う多角形を登録
					ret.push_back(cycle);
...
private:
	...
	int noiz;			//位置に、プラスマイナスnoizピクセルの誤差を付ける
	...
	void convert() {
		for (int i = 0; i < n; i++) {
			rot[i] = GetRand(628) / 100.0 - 3.14159265358979;	//ランダム回転です。回転なしにする場合はrot[i] = 0に設定してください。
			trans[i].x = GetRand(301) - 150;
			trans[i].y = GetRand(301) - 150;
			//opFlag[i] = ((GetRand(2) == 1) ? true : false);		//trueにするとピースiが反転します
		}
		noiz = 0;		//頂点座標に誤差をつけるパラメータです。noiz = 0とすると殆ど正確になり, noizを大きくすると頂点座標に誤差が加わります。
	}
	...
	void makePoly(int is_piece) {
		for (int i = 0; i < n; i++) {
			...
			poly[i] = polyIni[i];
			if (is_piece) {
				//y軸を対称として反転 (頂点列の向きは変化させない, ので頂点iが頂点n-1-iになる)
				if (opFlag[i]) {
					vector<Point> tmp;
...
int WINAPI WinMain(HINSTANCE,HINSTANCE,LPSTR,int) {
	...
	//第2引数に書いた場所に問題データ（新しい書式のもの）が書き込まれます。
	//ファイル名の仕様：
	//T…平行移動ありのときつける, R…回転ありのときつける, U…反転ありのときつける, _Noiz●…座標値に-●～+●の誤差をつけるときにつける。●=0のときは付けない。, △…番号タグ
	//[T][R][U][_Noiz●]△.txtのように書きます。△は任意の文字列。
	//詳しくは、ProbGeneクラスのconvert()関数を参照してください。opFlag[i] = trueになる可能性があれば、反転加工があるとみなしてください。
	CopyFile("probData.txt", "TR_sample1.txt", FALSE);	//FALSEにすると、ファイルが存在している場合に「上書きされる」。TRUEにすると「上書きされない」。
	DxLib_End();
	return 0;
}