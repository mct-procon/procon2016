...
bool SolverBase::canput(Poly poly) {
	...
	for (i = 0; i < pieces.size(); i++) if (pieces[i].is_putted && poly.is_contained(pieces[i])) return false;	//ピースに含まれたらダメ (<-バグっぽい)
...
void SolverBase::draw(bool is_draw_didnot_put) {
	...
	for (i = 0; i < pieces.size(); i++) {
		if (!pieces[i].is_putted && is_draw_didnot_put) {
			//DrawFormatString(100 + 200 * (i % 2), 100 + 25 * (i / 2), GetColor(255, 0, 0), "%.1f %.1f", pieces[i].get_point(0).real(), pieces[i].get_point(0).imag());
