...
bool game_loop() {
	...
	State state = State(pieces, wakus, solver.lastestMargedPolyIds, solver.scores);
	backup.push(state);
	...
	if (user_query.is_exist_dst == false) {
		vector<MoveNode> how_to_move = solver.connect_auto(true, 10);	//反転あり
...
int WINAPI WinMain(HINSTANCE,HINSTANCE,LPSTR arg,int) {
	...
	fnameList.push_back("Problem/sampleEx/problem.txt");
	fnameList.push_back("Problem/Waku_Square/ConvexAll/Less20Piece/NoOption/prob.txt");
	fnameList.push_back("NewProblem/TR_prob22.txt");
	fnameList.push_back("Problem/Research/LuckyPuzzle.txt");
	fnameList.push_back("C:/Users/NAOTTI/Documents/Visual Studio 2015/Projects/PuzzleSolver2016/PuzzleSolver2016/NewProblem/sample_scannerhand.txt");
