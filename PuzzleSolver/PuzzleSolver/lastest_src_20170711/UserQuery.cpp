...
void UserQuery::middle_push(int mouseX, int mouseY) {
	...
	if (drug_piece_id != -1 && drug_piece_id != poly_id) return;

	//ピースpoly_idをドラッグする
	drug_piece_id = poly_id;
	trans /= scale;
...
//プロセスメッセージ==0なら0, エスケープで抜けたならKEY_INPUT_ESCAPE, エンターで抜けたならKEY_INPUT_RETURN, を返す
int UserQuery::set_query() {
	init();
	while (ScreenFlip() == 0 && ProcessMessage() == 0 && ClearDrawScreen() == 0) {
		memcpy(bkey, key, 256);
		GetHitKeyStateAll(key);
		if (!bkey[KEY_INPUT_RETURN] && key[KEY_INPUT_RETURN]) return KEY_INPUT_RETURN;	//エンターで終了
		if (!bkey[KEY_INPUT_ESCAPE] && key[KEY_INPUT_ESCAPE]) return KEY_INPUT_ESCAPE;	//エスケープ
		...
		if (now_mouseInput & MOUSE_INPUT_MIDDLE) middle_push(mouseX, mouseY); else drug_piece_id = -1;
		...
		if (!bkey[KEY_INPUT_BACK] && key[KEY_INPUT_BACK]) {
			...
			solver.lastestMargedPolyIds = res.lastestMargedPolyIds;
			solver.scores = res.scores;
		...
		//Wキークリックで問題ファイルを書き換える。
		if (!bkey[KEY_INPUT_W] && key[KEY_INPUT_W]) {
			solver.print(read_file_name);
		}
