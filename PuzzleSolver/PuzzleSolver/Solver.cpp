...
vector<MoveNode> Solver::connect_auto(bool is_turn_ok, int max_moveList_size)
{
	...
	for (int i = 0; i < res.size(); i++) {
		...
		if (score > 0) {
			move_list.push_back(move_node);
		}
	}
	sort(move_list.begin(), move_list.end());	//スコアが大きい順にソート
	...
	for (int i = 0; i < move_list.size(); i++) {
		...
		MoveNode move_node = move_list[i];

		move(move_node);
		if (can_marge(move_node.src_poly_id, move_node.dst_poly_id, move_node.dst_is_piece)) { ret.push_back(move_node); }
		restore();
	}
