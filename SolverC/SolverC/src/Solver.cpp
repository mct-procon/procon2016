...
tuple<bool, Point, Point> Solver::move(int src_piece_id, int src_line_id, bool turnflag, int dst_line_id) {
	...
	Point dst_s = selected_lines[dst_line_id].start_point();
	Point dst_e = selected_lines[dst_line_id].end_point();
...
double Solver::evaluation(int gid) {
	...
	for (i = 0; i < selected_lines.size(); i++) {
		Point s = selected_lines[i].start_point();
		Point e = selected_lines[i].end_point();
	...
	for (i = 0; i < lines1.size(); i++) {
		for (j = 0; j < lines2.size(); j++) {
			eval += lines1[i].evaluation(lines2[j], 0.02, 7);
		}
	}
	return eval;
}
...
void Solver::select_line(complex<double> mousePoint) {
	...
	HalfLine line = HalfLine(lines[best].e, lines[best].s);		//辺の向きを逆にしておく
	vector<HalfLine>::iterator it = find(selected_lines.begin(), selected_lines.end(), line);
	if (it != selected_lines.end()) {
		int pos = (it - selected_lines.begin()) / sizeof(HalfLine);
...
void Solver::draw() {
	...
	for (int i = 0; i < selected_lines.size(); i++) {
		Point start = selected_lines[i].start_point() * scale;
		Point end   = selected_lines[i].end_point() * scale;
